...
def save_order():
    ...
    if not req_json:
        return jsonify(errno=RET.OK,errmsg='请求数据为空')
    # 进一步获取详细参数信息,house_id/start_date/end_date

    house_id = req_json.get('house_id')
    start_date_str = req_json.get('start_date')
    end_date_str = req_json.get('end_date')

    # 2.参数校验
    if not all([house_id,start_date_str,end_date_str]):
        return jsonify(errno=RET.OK,errmsg='参数不完整')
